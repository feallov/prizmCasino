...
async function handle(request: Request, env: any): Promise<Response> {
  ...
  if (url.pathname === '/api/me') {
    ...
    return json({
      user: { id: user.telegram_id, username: user.username, first_name: user.first_name, photo_url: user.photo_url, balance: user.balance, isAdmin: isAdmin(env, uid), last_daily: user.last_daily ?? 0, streak: user.streak ?? 0 },
      online: on?.c ?? 1,
  ...
  if (url.pathname === '/api/task_claim') {
    ...
    let ok = false;
    if (t.id === 'sub') {
      const sc2 = await checkSub(env, uid);
      ok = sc2.sub;
      if (!ok && sc2.err) return json({ error: 'not_ready', msg: sc2.err }, 400);
    } else if (t.id === 'first') {
  ...
  if (url.pathname.startsWith('/api/admin')) {
    ...
    if (url.pathname === '/api/admin/add') {
      ...
      let balance: number;
      if (amount > 0) balance = await credit(env, target, amount);
      else {
        await env.DB.prepare('UPDATE users SET balance = MAX(0, balance + ?) WHERE telegram_id = ?').bind(amount, target).run();
        const r: any = await env.DB.prepare('SELECT balance FROM users WHERE telegram_id = ?').bind(target).first();
        balance = r?.balance ?? 0;
      }
