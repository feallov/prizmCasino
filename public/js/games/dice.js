...
export function renderDice(app){
  ...
  slider.oninput = () => { if(!busy){ target = +slider.value; calc(); haptic(tg); } };
  tog[0].onclick = () => { if(!busy){ over = false; tog[0].classList.add('active'); tog[1].classList.remove('active'); calc(); } };
  tog[1].onclick = () => { if(!busy){ over = true; tog[1].classList.add('active'); tog[0].classList.remove('active'); calc(); } };
  calc();
  ...
  go.onclick = async () => {
    ...
    if (!d.ok){
      ...
      tog.forEach(t => t.disabled = false);
    ...
    const scr = setInterval(() => {
      ...
      if (ticks > 10){
        ...
        res.textContent = d.win ? `+${fmt(profit)} ₽` : `−${fmt(bet)} ₽`;
